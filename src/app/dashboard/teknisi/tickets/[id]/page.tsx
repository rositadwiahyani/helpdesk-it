  ...
  if (!ticket) {
    return (
      <div className="p-16 text-center">
        ...
        <Link href="/dashboard/teknisi/open-tickets" className="text-[var(--gold)] font-bold mt-2 inline-block">
          Back to Tickets
        </Link>
      </div>
    );
    ...
    <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-2 duration-300 p-6">
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-2 text-[13px] font-bold text-[var(--text-dim)]">
          <Link href="/dashboard/teknisi/open-tickets" className="hover:text-[var(--ink)] transition-colors">Tickets</Link>
          <span>/</span>
      ...
      <div className="flex items-center gap-1 border-b border-[var(--line)] bg-[var(--paper-2)]/30 px-4 pt-4 rounded-t-2xl border border-b-0">
        <button
          ...
          className={`px-5 py-2.5 text-[13.5px] font-bold rounded-t-lg border border-b-0 transition-colors ${activeTab === 'thread'
        ...
        <button
          ...
          className={`px-5 py-2.5 text-[13.5px] font-bold rounded-t-lg border border-b-0 transition-colors ${activeTab === 'tasks'
            ? 'bg-white border-[var(--line)] text-[var(--ink)]'
            : 'bg-transparent border-transparent text-[var(--text-dim)] hover:text-[var(--ink)] hover:bg-[var(--line-dark)]'
            }`}
      ...
      {activeTab === 'tasks' ? (
        ...
        <div className="flex flex-col gap-6">
          ...
          {ticket.conversations.map((msg: any) => {
            ...
            return (
              <div key={msg.id} className="flex flex-col gap-3">
                ...
                {/* Message Bubble Card */}
                <div className={`rounded-2xl border overflow-hidden shadow-sm transition-all ${isNote ? 'bg-amber-50/50 border-amber-200' : 'bg-blue-50/20 border-blue-100'
                  }`}>
                  {/* Message Header */}
                  <div className={`px-5 py-3 border-b flex items-center justify-between gap-4 text-xs font-bold text-[var(--text-dim)] ${isNote ? 'bg-amber-100/40 border-amber-200 text-amber-900' : 'bg-blue-100/20 border-blue-100 text-blue-900'
                    }`}>
                    <div className="flex items-center gap-2">
                      <span>ðŸ‘¤ {msg.sender}</span>
                      <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${isNote ? 'bg-amber-200/60 text-amber-900' : msg.role === 'admin' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-700'
                        }`}>
                ...
                {ticket.timeline
                  .filter((event: any) => event.timestamp === msg.timestamp)
                  .map((ev: any) => (
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl px-5 py-3 text-xs font-bold text-amber-800 flex items-center gap-2">
            âš ï¸ Ticket is assigned to {ticket.assignedTo || 'Support IT'}
          </div>
          ...
          <div className="bg-white rounded-2xl border border-[var(--line)] shadow-sm overflow-hidden flex flex-col">
            ...
            <div className="flex items-center border-b border-[var(--line-dark)] bg-slate-50 px-4">
              <button
                ...
                className={`px-4 py-3 text-xs font-bold border-b-2 transition-all ${formTab === 'note'
              ...
              <button
                ...
                className={`px-4 py-3 text-xs font-bold border-b-2 transition-all ${formTab === 'reply'
            ...
            <form onSubmit={handleSubmitAction} className="p-6 flex flex-col gap-4">
              ...
              >
                ðŸ“ Drop files here or <span className="text-[var(--gold-soft)] underline">choose them</span>
              </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-t border-[var(--line-dark)] pt-4 mt-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    type="submit"
                    className={`px-5 py-2 rounded-xl text-xs font-bold shadow-sm text-white transition-all ${formTab === 'note' ? 'bg-amber-600 hover:bg-amber-700' : 'bg-blue-600 hover:bg-blue-700'
                      }`}
                  >
