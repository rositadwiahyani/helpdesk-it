...
export default async function OperatorResolvedTicketsPage() {
    let apiData;
    try {
        const res = await fetchServer('/operator/tickets/resolved');
        apiData = res.data;
    } catch (err) {
    ...
    return (
        <div className="w-full h-full text-slate-800 font-sans p-6 md:p-10">
            <div className="mb-8 flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Tiket Selesai</h1>
                    <p className="text-sm text-slate-500 mt-1">Daftar tiket yang telah selesai (RESOLVED / CLOSED).</p>
                </div>
            </div>
