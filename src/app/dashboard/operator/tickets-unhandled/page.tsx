...
export default async function OperatorUnhandledTicketsPage() {
    let apiData;
    try {
        const res = await fetchServer('/operator/tickets/unhandled');
        apiData = res.data;
    } catch (err) {
        console.error("Error fetching unhandled tickets:", err);
        apiData = {
            tickets: [],
            slaConfigs: [],
            departments: [],
    ...
    return (
        <div className="w-full h-full text-slate-800 font-sans p-6 md:p-10">
            <div className="mb-8 flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Tiket Belum Ditangani</h1>
                    <p className="text-sm text-slate-500 mt-1">Daftar tiket yang sudah diverifikasi namun belum dikerjakan oleh teknisi.</p>
                </div>
            </div>
