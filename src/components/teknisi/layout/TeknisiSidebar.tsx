...
export default function TeknisiSidebar({ isOpen, setIsOpen }: TeknisiSidebarProps) {
  ...
  const MENU_ITEMS = [
    { name: 'Dashboard', path: '/dashboard/teknisi', icon: LayoutDashboard },
    { name: 'Semua Tiket', path: '/dashboard/teknisi/tickets', icon: Ticket },
    { name: 'Profil', path: '/dashboard/teknisi/profile', icon: User },
  ];
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto py-5 px-4 flex flex-col gap-1 overflow-x-hidden">
          {MENU_ITEMS.map((item) => {
            ...
            } else if (item.path === '/dashboard/teknisi/tickets') {
              isActive = pathname === '/dashboard/teknisi/tickets' || pathname?.startsWith('/dashboard/teknisi/tickets/');
            } else {
